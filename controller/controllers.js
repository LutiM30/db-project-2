...
exports.dashboard = async(req, res) => {
    ...
    let songs = await Song.find({}).sort({uploadedAt: -1}).populate("artist").populate("genre");
    let artists = await Artist.find({}).sort({name: 1});
    let genres = await Genre.find({}).sort({name: 1});

    res.render(req.user.userType.toLowerCase()==="admin" ? 'admindashboard' : "index", { 
        pageTitle: 'Dashboard', 
...
exports.addSong = async (req, res) => {
    try {
        ...
        const { title, artist, newArtist, genre, newGenre, price, releaseYear, songURL } = req.body;
...
function generateInvoice(order, song, res) {
    ...
    const values = [song.title, song.artist.name, song.genre.name, song.releaseYear, `${song.price} CAD`];
